...
- **Victron replenishment:** `/admin/replenishment` uses only local PostgreSQL data: cached Xero sales over 30 and 90 days, KZN stock, and unreceived inbound quantities. It groups replacement SKU families and recommends the current SKU using the higher daily demand rate, a 5-day supplier lead time, 2 days of safety stock, and a 14-day target cover. The report does not issue live Xero or Victron API calls.
