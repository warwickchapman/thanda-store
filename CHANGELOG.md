...
- The **Inbound** page now preserves the supplier order numbers in the transient Backorders snapshot and displays those orange backorder cards above ordinary inbound orders in **Expected orders**. Each backorder line can be cleared individually; there is no clear-all action for an order, and backorders cannot be accidentally received as inbound stock.
