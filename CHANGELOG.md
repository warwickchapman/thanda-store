...
- The **Inbound** page now preserves the supplier order numbers in the transient Backorders snapshot and displays those orange backorder cards above ordinary inbound orders in **Expected orders**. Backorders remain read-only and cannot be accidentally received as inbound stock.
