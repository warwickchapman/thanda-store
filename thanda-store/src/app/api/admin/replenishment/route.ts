...
export async function GET() {
  ...
  try {
    const [
      ...
      pool.query<NoteRow>(
        "SELECT UPPER(sku) AS sku, note FROM victron_replenishment_notes",
      ),
    ...
    const notesBySku = new Map(
      notes.rows.map((row) => [
        resolveFamily(victronStockSku(row.sku)),
        row.note,
      ]),
    );
    ...
    for (const row of sales.rows) {
      ...
      if (
        !group.lastSoldAt ||
        (row.last_sold_at && row.last_sold_at > group.lastSoldAt)
      )
    ...
    const items = [...groups.entries()]
      .flatMap(([family, group]) => {
        ...
        const currentProduct = [...group.products].sort(
          (left, right) =>
            Number(predecessorSkus.has(victronStockSku(left.sku))) -
              Number(predecessorSkus.has(victronStockSku(right.sku))) ||
            Number(left.sku.toUpperCase().endsWith("R")) -
    ...
    return NextResponse.json(
      { error: "Unable to load the replenishment report." },
      { status: 500 },
    );
  }
}
...
export async function PATCH(request: Request) {
  ...
  if (typeof body?.note === "string") {
    ...
    if (note.length > 2000)
      return NextResponse.json(
        { error: "A note can be at most 2,000 characters." },
        { status: 400 },
      );
    if (note)
      await pool.query(
    ...
    `,
      ...
      await pool.query(
        "DELETE FROM victron_replenishment_notes WHERE sku = $1",
        [sku],
      );
